...
function OverviewTab() {
    ...
    if (loading) {
        return <div className="space-y-4">{[1, 2, 3, 4].map(i => <div key={i} className="h-20 bg-gray-100 rounded-xl animate-pulse" />)}</div>
    }
    ...
    return (
        <div className="space-y-8">
            {/* Period Filters */}
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="flex flex-wrap items-center gap-3">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <KPICard title="Valor em Estoque" value={money(kpis?.totalValue)} subtext={`${kpis?.productsCount || 0} produtos • ${kpis?.totalItems || 0} peças`} icon={DollarSign} iconCls="text-emerald-600" colorCls="bg-emerald-50/50 border-emerald-100" tooltip="Soma do preço de venda de TODAS as peças disponíveis no estoque." />
                <KPICard title="Custo Imobilizado" value={money(kpis?.totalCost)} subtext="Valor de compra dos produtos" icon={Package} iconCls="text-purple-600" colorCls="bg-purple-50/50 border-purple-100" tooltip="Quanto você PAGOU por todas as peças que ainda tem em estoque." />
                <KPICard title="Markup Médio" value={`${averageMarkup.toFixed(0)}%`} subtext="Lucro bruto sobre o custo" icon={TrendingUp} iconCls="text-blue-600" colorCls="bg-blue-50/50 border-blue-100" tooltip="A média de lucro bruto sobre o custo de todo o seu estoque atual." />
                <KPICard title="Estoque Baixo" value={`${kpis?.lowStockCount || 0}`} subtext="Produtos com ≤ 2 unidades" icon={AlertTriangle} iconCls="text-amber-600" colorCls="bg-amber-50/50 border-amber-100" tooltip="Número de produtos que estão acabando (2 unidades ou menos)." />
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    <RankingList title="Por Categoria" icon={Tag} items={topSellers?.byCategory} nameKey="category" valueKey="quantity" valueSuffix="un" tooltip="Quais TIPOS de roupa vendem mais?" />
                    <RankingList title="Por Cor" icon={Palette} items={topSellers?.byColor} nameKey="color" valueKey="quantity" valueSuffix="un" tooltip="Quais CORES as clientes preferem?" />
                    <RankingList title="Por Tamanho" icon={Ruler} items={topSellers?.bySize} nameKey="size" valueKey="quantity" valueSuffix="un" tooltip="Quais TAMANHOS são mais pedidos?" />
                    <RankingList title="Mais Lucrativos" icon={DollarSign} items={topSellers?.byProfit} nameKey="name" valueKey="profit" valueSuffix="" tooltip="Lucro TOTAL acumulado por peça." />
                </div>
            </motion.div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <Card className="border shadow-sm">
                        ...
                        <CardContent className="p-0 max-h-[300px] overflow-y-auto">
                            {lowStock.length === 0 ? (
                                <div className="p-8 text-center text-gray-400 text-sm">Nenhum produto com estoque baixo.</div>
                            ) : (
                                <div className="divide-y divide-gray-50">
                                    {lowStock.map((item, i) => (
                                        <div key={i} className="px-4 py-3 flex items-center gap-3 hover:bg-gray-50/50">
                                            ...
                                            <span className={cn("text-xs font-bold px-2 py-1 rounded", item.stock === 0 ? "bg-red-100 text-red-600" : "bg-amber-100 text-amber-700")}>
                                                {item.stock} un
                                            </span>
                                        </div>
                                    ))}
                    ...
                    <Card className="border shadow-sm">
                            ...
                            <CardTitle className="text-sm font-bold text-[#4A3B32] flex items-center justify-between">
                                ...
                                <TooltipProvider>
                                    <Tooltip delayDuration={0}>
                                        ...
                                        <TooltipContent className="max-w-xs bg-[#2A211C] text-white p-2 text-xs">
                                            Produtos que estão no estoque há mais de 90 dias sem vender.
                                        </TooltipContent>
                                    </Tooltip>
                                </TooltipProvider>
                            </CardTitle>
                        ...
                        <CardContent className="p-4">
                            {deadStock?.count === 0 ? (
                                <div className="p-8 text-center text-gray-400 text-sm">Nenhum produto encalhado. Ótima notícia!</div>
                            ) : (
                                <div className="text-center">
                                    <div className="text-4xl font-display font-bold text-red-500 mb-2">{deadStock?.count || 0}</div>
                                    <p className="text-sm text-gray-500">produtos parados</p>
                                    <p className="text-xs text-gray-400 mt-1">≈ {money(deadStock?.totalValue)} em custo imobilizado</p>
                                </div>
                            )}
                        </CardContent>
                    </Card>
